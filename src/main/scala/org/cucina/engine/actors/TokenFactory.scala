...
case class CreateToken(processDefinition: ProcessDefinition, domainObject: Object) extends TokenMsg
/**
 * @author levinev
 */
class TokenFactory extends Actor {
  ...
  def receive = {
    case CreateToken(processDefinition, domainObject) => {
      ...
      tokenRepository ! new FindByDomain(processDefinition, domainObject, sender)
    }
