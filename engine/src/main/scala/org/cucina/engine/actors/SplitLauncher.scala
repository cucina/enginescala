...
class SplitLauncher(join: ActorRef, parentPc: ProcessContext) extends Actor {
  ...
  def receive = {
    ...
    case ec@ExecuteComplete(pc) =>
      ...
      launched -= 1
      if (launched == 0) {
        LOG.info("Last complete, notifying join " + join)
        join ! ExecuteComplete(parentPc)
        self ! PoisonPill
      }
